...
function getVedItemArticle(it) {
    if (it == null) return '';
    if (typeof it === 'string' || typeof it === 'number') { var s0 = String(it).trim(); return isVedJunk(s0) ? '' : s0; }
    if (Array.isArray(it)) {
        ...
        for (var i = 0; i < Math.min(it.length, 6); i++) {
            ...
            if (!isVedJunk(s)) return s;
        }
        return '';
    }
    // Объект: только ключи артикула (не путать с наименованием!)
    var keys = ['article', 'Артикул', 'Артикул сырья', 'Артикул товара', 'Код товара', 'art', 'sku', 'code', 'Модель', 'model'];
    ...
    // Fallback: глубокий поиск строк (только значения, похожие на артикул: не длинные, не кириллические названия)
    var found = [];
    function walk(o, depth) {
        ...
        for (var k in o) {
            ...
            if (keys.indexOf(k) !== -1) continue; // уже проверено
            var v = o[k];
            if (v == null) continue;
            if (typeof v === 'string' || typeof v === 'number') {
                var s = String(v).trim();
                if (isVedJunk(s)) continue;
                // Похоже на артикул: короткое, с цифрами/дефисами, или просто короткий код
                if (s.length <= 30 && (s.indexOf('-') !== -1 || /^[A-Za-z]{1,4}\d{2,}/.test(s) || /^\d{3,}$/.test(s) || !/[а-яА-ЯЁё]/.test(s))) found.push(s);
